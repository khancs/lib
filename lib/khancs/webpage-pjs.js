...
*/
(function(context) {
    var currentScript = context.document.currentScript; // Store the currently-executed script for future use
     ...
     function() {
        ...
        var runPJS = function(canvas, scriptText) {
            var instance = new context.Processing(canvas, function(env) {
                ...
                with (env) {
                    eval(scriptText); // Evaluate the script text - MAKE SURE it is trusted code!
                }
            });
        };
